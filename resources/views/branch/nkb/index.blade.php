        ...
        <div class="card-body">
            <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
                ...
                <a href="{{ route('nkb.create') }}" class="btn btn-primary btn-sm">
                    <i class="bi bi-plus-lg me-1"></i>Tambah NKB
                </a>
            </div>
                    ...
                    <tbody>
                        @forelse($items ?? [] as $item)
                            <tr>
                                ...
                                <td class="text-end">{{ $item->total_type_books ?? 0 }} / {{ number_format($item->total_exemplar ?? 0) }}</td>
                                <td class="text-center">
                                    <a href="{{ route('nkb.show', ['number' => $item->number]) }}"
                                        class="btn btn-sm btn-outline-primary">
                                        <i class="bi bi-eye me-1"></i>Detail
                                    </a>
                                    <a href="{{ route('nkb.print', ['number' => $item->number]) }}"
