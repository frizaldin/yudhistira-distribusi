<x-layouts>
    ...
    <div class="card mb-3">
        <div class="card-body">
            <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
                <div>
                    <strong>Tambah NKB Penyesuaian</strong><br />
                    <small class="text-muted">Cabang: GIP, Pengirim: PS00.</small>
                </div>
                <a href="{{ route('nkb_penyesuaian.index') }}" class="btn btn-outline-secondary btn-sm">
                    <i class="bi bi-arrow-left me-1"></i>Kembali
                </a>
            </div>

            <form action="{{ route('nkb_penyesuaian.store') }}" method="POST" id="formNkbPenyesuaian">
                @csrf

                {{-- ====== HEADER ====== --}}
                <div class="row g-3 mb-4">
                    <div class="col-md-3">
                        <label for="nota_kirim_cab" class="form-label">No. Nota Kirim / SJ <span class="text-danger">*</span></label>
                        <input type="text" name="nota_kirim_cab" id="nota_kirim_cab"
                            class="form-control @error('nota_kirim_cab') is-invalid @enderror"
                            value="{{ old('nota_kirim_cab') }}" required />
                        @error('nota_kirim_cab')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-3">
                        <label for="nppb" class="form-label">NPPB</label>
                        <input type="text" name="nppb" id="nppb"
                            class="form-control @error('nppb') is-invalid @enderror"
                            value="{{ old('nppb') }}" />
                        @error('nppb')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-3">
                        <label for="sj" class="form-label">SJ</label>
                        <input type="text" name="sj" id="sj"
                            class="form-control @error('sj') is-invalid @enderror"
                            value="{{ old('sj') }}" />
                        @error('sj')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <table class="table table-bordered table-sm align-middle mb-0" id="tabelBuku">
                        <thead class="table-light text-center">
                            <tr>
                                ...
                                <th style="min-width:250px" class="text-start">Kode Buku <span class="text-danger">*</span></th>
                                <th style="width:100px" class="text-end">Koli <span class="text-danger">*</span></th>
                                <th style="width:100px" class="text-end">Isi Koli <span class="text-danger">*</span></th>
                                <th style="width:100px" class="text-end">Pls/Exp <span class="text-danger">*</span></th>
    ...
    $(document).ready(function () {
        ...
        function initSelect2Baris(row) {
            ...
            // Setup onSelect to fill price automatically if possible
            $(sel).on('select2:select', function (e) {
                var data = e.params.data;
                // Unfortunately our generic products API might not return price in text format
                // So user needs to fill price manually or we would need a specific endpoint.
            });
        }
