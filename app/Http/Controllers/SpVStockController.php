...
class SpVStockController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($products as $product) {
            $cs = $centralStocks->get($product->book_code);
            $mut = $stockMutationsByBook->get($product->book_code);
            $rawStockPusat = (float) ($cs?->total_stock_pusat ?? 0) + (float) ($mut?->total_mutasi ?? 0);
