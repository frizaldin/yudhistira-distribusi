    ...
    <div class="card mb-3">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="card-title mb-0">Detail Nota Promosi</h5>
            ...
            <div class="row mb-4">
                <div class="col-md-12">
                    <table class="table table-sm table-borderless">
                        <tr>
                            <th width="200">No. Referensi (nota_kirim_promo)</th>
                            <td>: <strong>{{ $data->nota_kirim_promo }}</strong></td>
                        </tr>                        <tr>
                            <th>Cabang Pengirim (branch_sender)</th>
                            <td>: {{ $data->branch_sender ?? '-' }}</td>
                        </tr>                        <tr>
                            <th>Tgl Kirim (send_date)</th>
                            <td>: {{ $data->send_date ? \Carbon\Carbon::parse($data->send_date)->format('d/m/Y') : '-' }}</td>
                        </tr>                        <tr>
                            <th>Sales Code (sales_code)</th>
                            <td>: {{ $data->sales_code ?? '-' }}</td>
                        </tr>                        <tr>
                            <th>Pengirim (deliver_by)</th>
                            <td>: {{ $data->deliver_by ?? '-' }}</td>
                        </tr>                        <tr>
                            <th>Disetujui Oleh (approve_by)</th>
